...
bool FImage::writeImage(string file)	{
	...
	case FIT_UINT16:
		for (unsigned int y = 0; y < height; ++y)	{
			...
			for (unsigned int x = 0; x < width; ++x)	{
				bits[x] = (unsigned short) data[x + width * y];
			}
		}
		break;
	case FIT_FLOAT:
